...
Future<dynamic> register(RegisterModel user)async{
  ...
  try {
  Response response =await dio.post("https://localhost:7192/api/account/register",
  data:formData, 
       options: Options(
        headers: {
  ...
  RegExp emailRegExp = RegExp(r'Email: ([\w\.\+]+@[a-zA-Z_]+?\.[a-zA-Z]{2,6})');
  RegExp verificationCodeRegExp = RegExp(r'Your verification code is: (\d+)');

  // استخراج المعلومات باستخدام التعبيرات النمطية
  String id = idRegExp.firstMatch(response.data)?.group(1) ?? '';
  String userName = usernameRegExp.firstMatch(response.data)?.group(1) ?? '';
  String email = emailRegExp.firstMatch(response.data)?.group(1) ?? '';
  String verificationCode = verificationCodeRegExp.firstMatch(response.data)?.group(1) ?? '';
      ...
      if (e.response != null) {
        print("DioError response data: ${e.response?.data}");
         if (e.response?.statusCode == 400) {
          ...
          if (errorData is Map && errorData['errors'] != null && errorData['errors']['Email'] != null) {
